...
tbls_lvm <- tbl_fit_lvm %>% 
  map(
    ~ as_grouped_data(.x, groups = 'term_group') %>% 
      ...
      align(align = 'center', part = 'all') %>% 
      align(j = 1, align = 'left', part = 'all') %>% 
      add_footer_lines("Results are presented for participants who did not have stroke or coronary heart disease event from visit 1 to visit 3 and had predicted 30-year cardiovascular disease risk <10% at visit 1.")
  )

tbls_lvm$lvmi_height.complete_case
